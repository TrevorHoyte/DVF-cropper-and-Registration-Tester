...
//average stuff
		...
		try {
			...
			while (inputStream.hasNext()) {
				...
				try {
					...
					while (inputStream2.hasNext()) {
						...
						String DistwithID =  df.format(Distance) + ",x:,"+ XMtest +",y:,"+  YMtest + ",z:," +ZMtest
				...
				pw3.close();
					...
					File file3 =new File("FINALRESULTSMF.csv");
					FileWriter fw3 = new FileWriter(file3,true);
					PrintWriter pw3 = new PrintWriter(fw3);
			...
			*/
			...
			//////////////////STD
			String fileName69 = "FINALRESULTSMF.csv";
			File file69 = new File(fileName69);
			double std1=0;
			try {
				Scanner inputStream69 = new Scanner(file69);
				// make sure you skip the correct number of lines depending on where your data starts
				while (inputStream69.hasNext()) {
					String data69 =inputStream69.next();
					String[] values69 =data69.split(",");
					double point = Double.parseDouble(values69[0]);
				
					double std0 = (point-Average)*(point-Average);
					std1 += std0;
					}
				
				
				inputStream69.close();
				double std2 = std1/numbpointAV;
				double std3 = Math.sqrt(std2);
				...
				PrintWriter pw3 = new PrintWriter(fw3);
				pw3.println("Average Distance :"+Average);
				pw3.println("Standard Deviation:"+std3);
				pw3.close();
				
				System.out.println("Standard Deviation:"+std3);
				inputStream.close();
			}
				catch(FileNotFoundException e)
				{}
